...
void p9_client_cb(struct p9_client *c, struct p9_req_t *req)
{
	...
	if (req->flush_tag && (req->rc->id == P9_RFLUSH)) {
		/* flush receive path */
		...
		if (other_req->status != REQ_STATUS_FLSH) /* stale flush */
			...
			spin_unlock_irqrestore(&c->lock, flags);
			if (other_req->aio_cb == NULL)
				wake_up(other_req->wq);
			else
				p9_client_notify_aio(c, other_req);
		}
		if (other_req->aio_cb == NULL)
			p9_free_req(c, req);
...
int p9_client_create_aio(struct p9_fid *fid)
{
	int tag;
	struct p9_client *c;
	struct p9_req_t *req;
	...
	req = p9_tag_alloc(c, tag);
	if (IS_ERR(req)) {
		if (p9_idpool_check(tag, c->tagpool))
			p9_idpool_put(tag, c->tagpool);
		tag = PTR_ERR(req);
	}
...
EXPORT_SYMBOL(p9_client_create_aio);

int p9_client_destroy_aio(struct p9_fid *fid, int tag)
{
	int retval = 0;
	struct p9_req_t *req;
	struct p9_client *c;
...
EXPORT_SYMBOL(p9_client_destroy_aio);

int p9_client_post_aio(struct p9_fid *fid, int tag, u64 offset, u32 count,
		       char *data, p9_req_aio_cb_t cb, void *arg)
{
	int rsize, error = 0;
	struct p9_req_t *req;
	struct p9_client *c;
	...
	if (req->status < REQ_STATUS_RCVD && req->status != REQ_STATUS_ALLOC) {
		error = -EIO;
		goto bail_out;
	}
	...
	if (data == NULL) {
		...
		error = p9pdu_writef(req->tc, c->dotu, "dqd", fid->fid, offset,
				     rsize);
	} else {
		p9pdu_prepare(req->tc, tag, P9_TWRITE);
		error = p9pdu_writef(req->tc, c->dotu, "dqD", fid->fid, offset,
				     rsize, data);
	}
	p9pdu_finalize(req->tc);
...
EXPORT_SYMBOL(p9_client_post_aio);

int p9_client_notify_aio(struct p9_client *c, struct p9_req_t *req)
{
	u32 count = 0;
	void *data = NULL;
	int error = 0;
	...
	error = p9_check_errors(c, req);
	if (error)
		goto bail_out;

	switch (req->rc->id) {
	case P9_RREAD:
		error = p9pdu_readf(req->rc, c->dotu, "D", &count, &data);
		break;
	case P9_RWRITE:
		error = p9pdu_readf(req->rc, c->dotu, "d", &count);
		break;
	default:
		error = -EIO;
	}
...
bail_out:
	req->aio_cb(error, count, data, req->aio_arg);

	return 0;
}
EXPORT_SYMBOL(p9_client_notify_aio);
