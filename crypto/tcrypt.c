...
/* to test AES-SSK */
			if(template[i].klen!=0)
			{
#endif /* CONFIG_ARCH_PNX */
			ret = enc ?
			...
			{
				/* To test AES SSK, as the result will be different	  */
				/* from one BaseBand to another one, we test by		  */
				/* encrypting then decrypting, then testing the result is */
				/* equivalent to the input. 				  */
				
				
				switch (crypto_ablkcipher_encrypt(req) ) {
				case 0:
					break;
				case -EINPROGRESS:
				case -EBUSY:
				...
				switch (crypto_ablkcipher_decrypt(req) ) {
				case 0:
					break;
				case -EINPROGRESS:
				case -EBUSY:
...
static void do_test(void)
{
	...
	case 99:
		...
		printk("TEST of ST-Ericsson / PNX platforms / AES driver\n");
		test_cipher("ecb(aes)", ENCRYPT, aes_enc_tv_template,
			    AES_ENC_TEST_VECTORS);
		test_cipher("ecb(aes)", DECRYPT, aes_dec_tv_template,
			    AES_DEC_TEST_VECTORS);
		...
		printk("TEST of ST-Ericsson / PNX platforms / AES driver, with SSK\n");
		test_cipher("ecb(aes)", ENCRYPT, aes_ssk_tv_template,
			    AES_SSK_TEST_VECTORS);

		printk("TEST of ST-Ericsson / PNX platforms / SHA1 driver\n");
