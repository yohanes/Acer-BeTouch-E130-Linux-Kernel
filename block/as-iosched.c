...
/* ACER Jen chang, 2010/06/09, IssueKeys:A41.B-1304, Disable BUG_ON check to avoid kernel panic when removing sd card whihc is not unmounted { */
//	BUG_ON(!list_empty(&ad->fifo_list[REQ_SYNC]));
//	BUG_ON(!list_empty(&ad->fifo_list[REQ_ASYNC]));
	if(!list_empty(&ad->fifo_list[REQ_SYNC]))
		printk("--BUG_ON(!list_empty(&ad->fifo_list[REQ_SYNC]));--\n");
	if(!list_empty(&ad->fifo_list[REQ_ASYNC]))
		printk("--BUG_ON(!list_empty(&ad->fifo_list[REQ_ASYNC]));--\n");
