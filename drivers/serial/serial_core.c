...
#define RELEVANT_IFLAG(iflag)	((iflag) & (IGNBRK|BRKINT|IGNPAR|PARMRK|INPCK))
	...
	if (!(old_termios->c_cflag & CRTSCTS) && (cflag & CRTSCTS)) {
		spin_lock_irqsave(&state->port->lock, flags);
		/**
		 * CTS flow control is not necessarily handled at this level
		 * even if cflag & CRTSCTS (eg. when AFE is enabled). Hence,
		 * check for UIF_CTS_FLOW as well.
		 */
/*		if (!(state->port->ops->get_mctrl(state->port) & TIOCM_CTS)) {*/
		if ((state->info.flags & UIF_CTS_FLOW) && !(state->port->ops->get_mctrl(state->port) & TIOCM_CTS)) {
			tty->hw_stopped = 1;
...
static int uart_open(struct tty_struct *tty, struct file *filp)
{
	 ...
	 */
	if (state->count == 1 && !uart_console(state->port))
