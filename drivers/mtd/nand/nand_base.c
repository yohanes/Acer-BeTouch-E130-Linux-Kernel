...
retry:
	...
	DEBUG (MTD_DEBUG_LEVEL0, "original bad block# %d with "
	  " offset 0x%08x, translate_block_ofs 0x%08x\n",
		orig_block, (unsigned int)ofs,
		(unsigned int)translate_block_ofs);
...
static int nand_default_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (chip->options & NAND_USE_FLASH_BBT){
		...
		if (ret==0){
			/* no error */
			ret=get_bbm_index(mtd, chip->bbm);
			if (ret >= 0){
				/* index OK */
				chip->bbm[block]=ret;
				ret=nand_update_bbm(mtd, ofs);
			}
		}
#endif
	}
 ...
 * by a good block from reserved pool.*/
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	if (nand_isbad_bbt (mtd, from, 0)) {
		...
		if (from_rb != from) {
			...
			DEBUG(MTD_DEBUG_LEVEL0, "%s: badblock offset:0x%08x, "
				"replblock offset:0x%08x\n", __func__,
	...
	while(1) {
		...
		if (!(page & blkcheck)) {
			...
			if (nand_isbad_bbt (mtd, (realpage << pg_shift), 0)) {
				from_rb = nand_translate_block(mtd,
						(realpage << pg_shift));
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	while(1) {
		...
		if (!(page & blkcheck)) {
			...
			/* check, if the given start page is in bad block */
			if (nand_isbad_bbt(mtd,
					(realpage << chip->page_shift),	0)) {
				from_rb = nand_translate_block(mtd,
					(realpage << chip->page_shift));
				if (from_rb != (realpage << chip->page_shift)) {
					realpage = (int) (from_rb >>
							chip->page_shift);
					block_replace_flag = 1;
				}
			}
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	if (nand_isbad_bbt (mtd, to, 0)) {
		...
		if (to_rb != to) {
			...
			__func__, (unsigned int)to, (unsigned int)to_rb);
	...
	while(1) {
		...
		if (!(realpage & blockmask)) {
			...
			if (nand_isbad_bbt(mtd, (realpage <<
						chip->page_shift), 0)) {
		...
		if (nand_isbad_bbt(mtd, (page <<
			...
			if (instr->addr == orig_addr) {
				printk(KERN_WARNING "nand_erase: attempt to "
					"erase a bad block at page 0x%08x\n",
				page);
				instr->state = MTD_ERASE_FAILED;
				goto erase_exit;
			}
			block_replace_flag = 1;
			DEBUG(MTD_DEBUG_LEVEL0, "nand_erase: "
				"badblck at 0x%08x replced by 0x%08x\n",
				(unsigned int) (page << chip->page_shift),
				(unsigned int) instr->addr);

			/* updating page offset
			 * with the replacement page offset */
			page = (int) (instr->addr >> chip->page_shift);
		}
...
#if defined(CONFIG_MTD_NAND_BBM)
		nand_update_bbm(mtd, rewrite_bbt[chipnr]);
#endif
	}
 ...
 * A bad block is checked in nand_do_read_ops so if nand_bbt.c
 * uses mtd->read_oob.
 ...
 */
int nand_read_raw(struct mtd_info *mtd, uint8_t *buf,
		loff_t from, size_t len, size_t ooblen)
{
						  ...
						  int busw, int *maf_id)
{
  ...
  /* ACER Erace.Ma@20100525, provide NAND device Manufacturer ID for AP*/
  {
    extern char nandID[10];
    memset(nandID,0x00,10);
    snprintf(nandID,10,"%s",nand_manuf_ids[maf_idx].name);
  }
  /* End ACER Erace.Ma@20100525*/
