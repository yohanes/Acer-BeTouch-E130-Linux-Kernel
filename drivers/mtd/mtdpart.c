		...
		uint64_t cur_offset)
{
	 ...
	 * block_isbad may use uninitialized rsvdblock_offset before
	 * 'rsv' partition is added.
	*/
	if (master->block_isbad) {
		uint64_t offs = 0;
	...
	if  (master->type == MTD_NANDFLASH) {
		...
		if (strcmp(slave->mtd.name, "rsv")){
			panic("%s's \"rsv\" partition does not exist,"
			   " NAND Bad Block can't be handled!\n", master->name);
		}

		rsvdblock_offset = slave->offset;
		printk(KERN_INFO "%s's capacity:%lld, "
				"reserved block offset:0x%08lx\n",
				master->name, (master->size >> 20),
				rsvdblock_offset);

		/*
		 * check the block number for the BBT/BBM reserved block pool..
		 * reserved block number for NAND flash should be 2% of the
		 * total blocks, plus:
		*	add extra: 2 BBT and 2 BBMs blocks.
		* how to calculate BBMs:
		* 	- 2 bytes for each block,
		* 	- bbmblocks = (totalblock*2)/blocksize;
		*/
		mtdblocks=mtd_div_by_eb(master->size,master);
		bbmblocks= DIV_ROUND_UP(mtdblocks*2,master->erasesize);
		rsvdblocks = DIV_ROUND_UP(mtdblocks*2,
				100) + 2 + (bbmblocks*2);

		if (rsvdblock_offset != (master->size -
					(rsvdblocks*(master->erasesize))))
			panic("%s:0x%08llx's rsv partition "
				"must start at offset:0x%08llx\n",
				master->name, master->size,
				master->size-(rsvdblocks*(master->erasesize)));
	}
#endif
