...
binder_get_ref_for_node(struct binder_proc *proc, struct binder_node *node)
{
	...
	if (node) {
		...
		//dark mark 2010.05.14 for tracking binder panic issue
		//if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
			printk(KERN_INFO "binder: %d new ref %d desc %d for "
				"dead node\n", proc->pid, new_ref->debug_id,
				new_ref->desc);
...
binder_delete_ref(struct binder_ref *ref)
{
	...
	//dark add 2010.05.14 for binder issue {
	if (!hlist_unhashed(&ref->node_entry)) 
	{
	  hlist_del(&ref->node_entry);
	}
	else
	{
		printk(KERN_INFO "node_entry prev = null binder: %d delete ref %d desc %d for "
			"node %d\n", ref->proc->pid, ref->debug_id,
			ref->desc, ref->node->debug_id);
	}
	//dark end }
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	if (proc->pages) {
		...
		for (i = 0; i < proc->buffer_size / PAGE_SIZE; i++) {
			if (proc->pages[i]) {
        //dark modify 2010.06.03 to fix binder memory corruption {
