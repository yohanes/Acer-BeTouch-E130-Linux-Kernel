...
static int acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if ((acm->port_handshake_bits & ACM_CTRL_DTR)
			!= (acm->port_handshake_bits_last & ACM_CTRL_DTR))

			acm->port.send_carrier_detect(&acm->port,
				acm->port_handshake_bits & ACM_CTRL_DTR);

		acm->port_handshake_bits_last = acm->port_handshake_bits;
...
int __init acm_bind_config(struct usb_configuration *c, u8 port_num)
{
	...
	acm->port_handshake_bits_last = 0;
